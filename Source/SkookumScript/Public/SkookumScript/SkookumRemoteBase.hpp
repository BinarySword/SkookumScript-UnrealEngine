...
// Skookum IDE.
#if (SKOOKUM & SS_DEBUG) && !defined(SKOOKUM_IDE_EMBEDDED)
...
//   SSLocale_embedded - Local Skookum environment debugged/controlled by its own local embedded IDE (not online)
//   SSLocale_runtime  - Client Skookum environment/runtime debugged/controlled by remote IDE
//   SSLocale_ide      - IDE remotely debugging/controlling client Skookum environment/runtime
//   
// and which side a given command should run on: local, remote or both.
//   SSLocale_local    - run command locally: translates to embedded, runtime or IDE as needed
//   SSLocale_remote   - run command remotely: translates to embedded, runtime or IDE as needed
//   SSLocale_all      - run command both locally and remotely
enum eSSLocale
  {
  // Actual codes
  SSLocale_embedded = 0x0, 
  SSLocale_runtime  = 1 << 0,
  SSLocale_ide      = 1 << 1,

  // Virtual codes
  SSLocale_local    = 1 << 2,  // Translated to embedded, runtime or IDE as needed
  SSLocale_remote   = 1 << 3,  // Translated to embedded, runtime or IDE as needed

  SSLocale_all      = SSLocale_runtime | SSLocale_ide | SSLocale_local | SSLocale_remote
  };
...
enum eSSRemoteFlag
  {
  SSRemoteFlag_none        = 0x0, 
  SSRemoteFlag_symbol_db   = 1 << 0  // Remote side has a symbol database (Symbols need to be updated, less or no symbol id translation required)
  };

const uint32_t SSRemote_version_byte_size       = 9u;   // command(4) + version(1) + authorization(4)
const uint32_t SSRemote_version_reply_byte_size = 13u;  // command(4) + version(1) + authorization(4) + client_flags(4)
...
class SkookumRemoteBase
  ...
  // Common Methods
    ...
    eSSLocale    get_mode() const                                 { return m_mode; }
    virtual void set_mode(eSSLocale mode);
    ...
    bool         is_embedded() const                              { return (this == nullptr) || (m_mode == SSLocale_embedded); }
    bool         is_remote_ide() const                            { return this && (m_mode == SSLocale_ide); }
    bool         is_remote_runtime() const                        { return this && (m_mode == SSLocale_runtime); }
    bool         is_connecting() const                            { return this && (m_connect_state <= ConnectState_authenticated); }
    virtual bool is_connected() const                             { return this && (m_mode != SSLocale_embedded); }
    bool         is_symbol_db_remote() const                      { return this && ((m_remote_flags & SSRemoteFlag_symbol_db) != 0u); }
  ...
  // Class Methods
    ...
    static eSSLocale locale_as_virtual(eSSLocale locale);
    static eSSLocale locale_as_actual(eSSLocale locale);
  ...
  // Data Members
    ...
    // Flags from remote side - see eSSRemoteFlag
