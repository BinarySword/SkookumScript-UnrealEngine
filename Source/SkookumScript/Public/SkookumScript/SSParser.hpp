...
#ifndef __SSPARSER_HPP
#define __SSPARSER_HPP
...
#include "SkookumScript/SSTypedContext.hpp"
...
class SSClassUnion;
class SSCode;
class SSConcurrentBranch;
class SSConcurrentRace;
class SSConcurrentSync;
class SSConditional;
class SSCoroutineBase;
class SSCoroutineFunc;
class SSCoroutineCall;
class SSDivert;
class SSGroupParam;
class SSIdentifier;
class SSIdentifierMember;
class SSInvocation;
class SSInvokeBase;
class SSInvokeCascade;
class SSInvokeRace;
class SSInvokeSync;
class SSLiteral;
class SSLiteralClosure;
class SSLiteralList;
class SSLoop;
class SSLoopExit;
class SSMetaClass;
...
class SSParser : public AString
  ...
  // Nested Structures
    ...
    // Used by SSParser::Args.m_flags below.
    enum eArgFlag
      {
    ...
    struct Args
      ...
      // Data Members
        ...
        // IN Data
          ...
          SSClassDescBase * m_desired_type_p;
        ...
        // OUT Data
          ...
          SSClassDescBase * m_type_p;


      // Methods

        SS_NEW_OPERATORS(SSParser::Args);
  ...
  // Common Methods
    ...
    SS_NEW_OPERATORS(SSParser);

    SSParser(const AString & str);
    SSParser(const char * cstr_p, uint32_t length = ALength_calculate, bool persistent = true);

    SSParser & operator=(const AString & str)              { AString::operator=(str); return *this; }
    SSParser & operator=(const SSParser & parser)          { AString::operator=(parser); return *this; }

    SSTypeContext & get_context() const                    { return m_context; }
    eSSMember       get_member_type() const                { return m_member_type; }
    void            reset_scope(SSClassUnaryBase * scope_p = nullptr, const ASymbol & scope_name = ASymbol::ms_null) const;
    void            set_class_scope(SSClassUnaryBase * scope_p = nullptr) const;
    void            set_member_type(eSSMember type) const  { m_member_type = type; }
    void            set_scope(SSClassUnaryBase * scope_p = nullptr, const ASymbol & scope_name = ASymbol::ms_null) const;
  ...
  // Methods
    ...
    bool              parse_class_meta_source(SSClass * scope_p, Args & args = ms_def_args.reset());
    eResult           parse_data_members_source(SSClassUnaryBase * scope_p, uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, bool append_to_class_b = true);
    SSMethodBase *    parse_method_source(const ASymbol & name, SSClassUnaryBase * scope_p, Args & args = ms_def_args.reset(), bool append_to_class_b = true);
    SSCoroutineBase * parse_coroutine_source(const ASymbol & name, SSClassUnaryBase * scope_p, Args & args = ms_def_args.reset(), bool append_to_class_b = true);
    ...
    // Class Result Type Methods
      ...
      // $Note - CReis Methods that determine a resulting class type pass use SSParser::Args.
      // Args are passed in non-const so that coercion can be used to create simple Args
      // structures.  Additionally SSParser::ms_def_args is used as a default so that the
      ...
      SSExpressionBase * parse_binding(Args & args = ms_def_args.reset()) const;
      SSLiteralClosure * parse_closure(Args & args = ms_def_args.reset(), bool allow_inline = false) const;
      SSCode *           parse_code_block(Args & args = ms_def_args.reset(), eSSInvokeTime desired_exec_time = SSInvokeTime_any, eResultDesired result = ResultDesired_true) const;
      SSExpressionBase * parse_code_block_optimized(Args & args = ms_def_args.reset(), eSSInvokeTime desired_exec_time = SSInvokeTime_any, eResultDesired result = ResultDesired_true) const;
      SSExpressionBase * parse_expression(Args & args = ms_def_args.reset(), eSSInvokeTime desired_exec_time = SSInvokeTime_any) const;
      SSExpressionBase * parse_instantiate_or_list(Args & args = ms_def_args.reset()) const;
      SSLiteralList *    parse_literal_list(Args & args = ms_def_args.reset(), SSTypedClass * list_class_p = nullptr, bool item_type_b = false, SSMethodCall * ctor_p = nullptr) const;
      SSLiteral *        parse_literal_number(Args & args = ms_def_args.reset(), bool * simple_int_p = nullptr) const;
      SSLoopExit *       parse_loop_exit(Args & args = ms_def_args.reset()) const;
      SSMethodBase *     parse_method(Args & args = ms_def_args.reset(), const ASymbol & name = ASymbol::get_null(), eSSInvokeTime desired_exec_time = SSInvokeTime_any, bool append_to_class_b = true) const;
      SSCoroutineBase *  parse_coroutine(Args & args = ms_def_args.reset(), const ASymbol & name = ASymbol::get_null(), bool append_to_class_b = true) const;
      bool               parse_temporary(Args & args = ms_def_args.reset(), ASymbol * ident_p = nullptr, SSExpressionBase ** expr_pp = nullptr) const;


    // Simple Parse Methods

      eResult parse_class(                uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSClass ** class_pp = nullptr) const;
      eResult parse_class_desc(           uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSClassDescBase ** type_p = nullptr) const;
      eResult parse_class_instance(       uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSClassUnaryBase ** class_pp = nullptr, bool * item_type_b_p = nullptr) const;
      eResult parse_class_typed(          uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSTypedClass ** tclass_p = nullptr, bool * item_type_b_p = nullptr) const;
      eResult parse_class_union(          uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSClassUnion ** union_p = nullptr) const;
      eResult parse_class_meta(           uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSMetaClass ** mclass_p = nullptr) const;
      eResult parse_class_scope(          uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSClass ** qual_scope_pp = nullptr, SSClassDescBase * scope_p = nullptr) const;
      ...
      eResult parse_literal_integer(      uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSIntegerType * int_p = nullptr, uint32_t * radix_p = nullptr) const;
      eResult parse_literal_real(         uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSRealType * real_p = nullptr, bool int_as_real_b = true) const;
    ...
    // Preparse Methods - Partially parses code for context for later full parse.
      ...
      SSMethodFunc *    preparse_method_source(const ASymbol & name, SSClassUnaryBase * scope_p, Args & args = ms_def_args.reset()) const;
      SSCoroutineFunc * preparse_coroutine_source(const ASymbol & name, SSClassUnaryBase * scope_p, Args & args = ms_def_args.reset()) const;
    ...
    // Identification Methods  - Quickly identifies/categorizes a section of code without necessarily doing a full analysis.
      ...
      eIdentify identify_class(      uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, AString * class_name_p = nullptr, SSClass ** class_pp = nullptr) const;
      eSSMember identify_member_filename(SSQualifier * ident_p = nullptr, bool * class_member_p = nullptr, bool optional_scope = true) const;
      eResult   identify_member_name(SSMemberInfo * member_p, uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr) const;
  ...
  // Class Methods
    ...
    static AFlagSet32 & get_default_flags()                                  { return ms_default_flags; }
           bool         ensure_exec_time(const SSExpressionBase & expr, Args & args, eSSInvokeTime desired_exec_time) const;
    ...
    static eResult      invoke_script(const AString & code, AString * result_str_p = nullptr, SSInstance ** result_pp = nullptr, SSInstance * instance_p = nullptr, bool print_info = true);
  ...
  // Internal Nested Structures
    ...
    struct InvokeArgsInfo
      ...
      // Data Members
        ...
        Args * m_args_p;
        SSClassDescBase * m_final_rcvr_type_p;
        const SSParameters * m_params_p;
        SSParameterBase ** m_plist_pp;
        SSParameterBase * m_param_p;

        SSLiteralList * m_group_arg_p;

        APCompactArray<SSExpressionBase> * m_arg_exprs_p;
      ...
      InvokeArgsInfo(Args * args_p, const SSParameters * params_p, APCompactArray<SSExpressionBase> * arg_exprs_p, bool implicit_arg1_b);
      ~InvokeArgsInfo();
  ...
  // Internal Methods
    ...
    SSMethodBase * get_method_inherited(SSClassDescBase * class_p, const ASymbol & method_name) const;

    // Internal Interface/Parameter Methods

      eResult parse_parameters(           uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSParameters * params_p = nullptr, uint32_t flags = ParamFlag__default) const;
      eResult parse_parameter(            uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSParameterBase ** param_new_pp = nullptr) const;
      eResult parse_parameter_specifier(  uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSTypedName * tname_p = nullptr, uint32_t param_flags = ParamFlag__default) const;
      eResult parse_parameter_unary(      uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSUnaryParam * uparam_p = nullptr) const;
      eResult parse_parameter_group(      uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSGroupParam * vparam_p = nullptr) const;
      eResult parse_param_append(         uint32_t start_pos, uint32_t * end_pos_p, SSParameters * params_p, uint32_t param_flags) const;
      eResult parse_param_return_append(  uint32_t start_pos, uint32_t * end_pos_p, SSParameters * params_p, uint32_t param_flags) const;

      eResult preparse_param_append(      uint32_t start_pos, uint32_t * end_pos_p, SSParameters * params_p, uint32_t param_flags) const;
      eResult preparse_parameter(uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr, SSParameterBase ** param_new_pp = nullptr) const;
      eResult preparse_parameter_unary(SSUnaryParam * uparam_p, uint32_t start_pos = 0u, uint32_t * end_pos_p = nullptr) const;

      void    parameters_context(const SSParameters & params, Args * result_type_p = nullptr) const;
      bool    parameters_typecheck(Args & args, SSParameters * params_p, bool check_result = true) const;


    // Internal Class Result Type Methods

      SSExpressionBase *   parse_expression_alpha(Args & args) const;
      SSCase *             parse_case_tail(Args & args = ms_def_args.reset()) const;
      SSConditional *      parse_conditional_tail(Args & args = ms_def_args.reset()) const;
      SSLoop *             parse_loop_tail(Args & args) const;
      SSConcurrentSync *   parse_concurrent_sync_block(Args & args) const;
      SSConcurrentRace *   parse_concurrent_race_block(Args & args) const;
      SSConcurrentBranch * parse_concurrent_branch_block(Args & args) const;
      SSDivert *           parse_divert_block(Args & args) const;
      SSObjectId *         parse_object_id_tail(Args & args, SSClass * class_p = nullptr) const;
      SSInvocation *       parse_prefix_operator_expr(const ASymbol & op_name, Args & args) const;
      bool                 parse_statement_append(Args & args, SSCode * code_p, eSSInvokeTime desired_exec_time = SSInvokeTime_any) const;
      bool                 parse_temporary_append(Args & args, SSCode * code_p) const;
      ...
      // entry and are set to resulting type on exit
        ...
        SSBind *             parse_bind(Args & args, SSExpressionBase * receiver_p) const;
        SSExpressionBase *   parse_class_cast(Args & args, SSExpressionBase * receiver_p) const;
        SSExpressionBase *   parse_class_conversion(Args & args, SSExpressionBase * receiver_p) const;
        SSIdentifierMember * parse_data_accessor(Args & args, SSExpressionBase * owner_p) const;
        SSExpressionBase *   parse_expression_string(Args & args, SSExpressionBase * receiver_p) const;
        SSInvokeCascade *    parse_invoke_cascade(Args & args, SSExpressionBase * receiver_p) const;
        SSExpressionBase *   parse_invoke_instantiate(Args & args, SSExpressionBase * receiver_p) const;
        SSInvokeSync *       parse_invoke_apply(Args & args, SSExpressionBase * receiver_p) const;
        SSInvokeRace *       parse_invoke_race(Args & args, SSExpressionBase * receiver_p) const;
        SSExpressionBase *   parse_invoke_operator(Args & args, SSExpressionBase * receiver_p) const;
        SSInvocation *       parse_invoke_index_operator(Args & args, SSExpressionBase * receiver_p) const;
        SSMethodCall *       parse_invoke_method(Args & args) const;
        SSMethodCall *       parse_invoke_method_arg1(Args & args, SSMethodBase * method_p, SSExpressionBase * arg1_p) const;
        SSMethodCall *       parse_invoke_ctor(Args & args) const;
        SSInvokeBase *       parse_invoke_selector(Args & args, bool test_op) const;
        SSCoroutineCall *    parse_invoke_coroutine(Args & args) const;
        SSExpressionBase *   parse_modifier_tail(Args & args, SSExpressionBase * expr_p) const;
        SSMethodCall *       parse_operator_call(Args & args) const;

        bool      parse_invoke_args_arg1(Args & args, SSExpressionBase * arg1_p, APCompactArray<SSExpressionBase> * args_p = nullptr, const SSMethodBase * method_p = nullptr) const;
        bool      parse_invoke_args(Args & args, APCompactArray<SSExpressionBase> * args_p = nullptr, APCompactArray<SSIdentifier> * ret_args_p = nullptr, const SSParameters * params_p = nullptr, eInvokeBrackets brackets = InvokeBrackets_opt_args0_closure, bool implicit_arg1_b = false) const;
        ...
        eResult   parse_invoke_return_args(uint32_t start_pos, uint32_t * end_pos_p, APCompactArray<SSIdentifier> * ret_args_p, const SSClassUnaryBase * receiver_type_p, const SSParameters * params_p, uint32_t bracket_flags) const;

    // Internal Simple Parse Methods

      ASymbol as_symbol(                uint32_t start_pos, uint32_t end_pos) const;
      bool    ensure_expr_effect(const SSExpressionBase * expr_p, uint32_t * pos_p, Args & args) const;
      bool    is_constructor(           uint32_t start_pos = 0u) const;
      eResult parse_digits_lead(        uint32_t start_pos, uint32_t * end_pos_p, SSIntegerType * int_p) const;
      ...
      eResult parse_named_specifier(    uint32_t start_pos, uint32_t * end_pos_p = nullptr, const SSParameters * params_p = nullptr, uint32_t * arg_idx_p = nullptr, SSParameters::eType param_type = SSParameters::Type_send) const;

      SSClassDescBase * identifier_desired_type(SSIdentifier * identifier_p, SSClassDescBase * identifier_type_p, SSClassDescBase * context_type_p) const;
      eResult           identifier_validate_bind(SSExpressionBase * identifier_p) const;
      eResult           identifier_validate_bind_type(SSIdentifier * identifier_p, SSClassDescBase * old_type_p, SSClassDescBase * new_type_p) const;


  // Data Members

    // Parse flags - see SSParser::eFlag
    AFlagSet32 m_flags;

    // Member type being parsed
    mutable eSSMember m_member_type;

    // Scope context of current parse
    mutable SSTypeContext m_context;


  // Class Data Members

    // Initial flags for new SSParser objects if flags are not otherwise specified.
    ...
    // Default arguments to use if no SSParser::Args structure is supplied.
...
#ifndef A_INL_IN_CPP
  #include "SkookumScript/SSParser.inl"
#endif


#endif  // __SSPARSER_HPP
