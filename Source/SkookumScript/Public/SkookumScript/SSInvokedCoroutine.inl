...
#include "SkookumScript/SSCoroutineCall.hpp"
...
// # Author(s): Conan Reis
A_INLINE SSInvokedCoroutine::operator const ASymbol & () const
  {
...
// # See:      <
// # Author(s): Conan Reis
A_INLINE bool SSInvokedCoroutine::operator==(const SSInvokedCoroutine & icoroutine) const
  {
...
// # Author(s): Conan Reis
A_INLINE void SSInvokedCoroutine::stop_tracking()
  {
  if (m_flags & Flag_tracked_mask)
    {
    SSMind * mind_p = m_mind_p;
...
// # See:      set_name()
// # Author(s): Conan Reis
A_INLINE const ASymbol & SSInvokedCoroutine::get_name() const
  {
  return m_coroutine_p->get_name();
  }
...
// # Author(s): Conan Reis
A_INLINE const SSCoroutineBase & SSInvokedCoroutine::get_coroutine() const
  {
...
// #Author(s) Conan Reis
A_INLINE SSInvokedCoroutine * SSInvokedCoroutine::pool_new(SSCoroutineBase * coroutine_p)
  {
  SSInvokedCoroutine * icoro_p = get_pool().pop();
...
// # Examples: SSInvokedCoroutine::pool_delete(icoroutine_p);
// # Author(s): Conan Reis
A_INLINE void SSInvokedCoroutine::pool_delete(SSInvokedCoroutine * icoroutine_p)
  {
  ...
  icoroutine_p->AListNode<SSInvokedBase>::remove();
  icoroutine_p->m_ptr_id = AIdPtr_null;

  SSObjectBase * scope_p = icoroutine_p->m_scope_p;

  if (scope_p)
    {
    static_cast<SSInstance *>(scope_p)->dereference();
    }
...
// # Author(s):  Conan Reis
A_INLINE void SSInvokedCoroutine::set_coroutine(const SSCoroutineBase & coroutine)
  {
