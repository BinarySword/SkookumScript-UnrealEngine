...
bool ADebug::resolve_error(
  ...
  bool               nested_ex    = std::uncaught_exception();
  AErrorOutputBase * err_output_p = Agog::on_error_pre(nested_ex);	// See _AgogCoreDefaults.cpp

  debug_break = (err_output_p && !nested_ex)
    ? err_output_p->determine_choice(msg, &action)  // Use supplied error output object
    : determine_choice(msg, &action);
...
bool ADebug::determine_choice(
  ...
  *action_p = (nested_ex || (msg.m_err_level <= AErrLevel_notify)) ? AErrAction_ignore : AErrAction_quit;
