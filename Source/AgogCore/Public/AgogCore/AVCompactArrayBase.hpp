...
//---------------------------------------------------------------------------------------
// Applies the supplied invoke_obj to elem_count elements starting at index
//             pos.
// Arg         invoke_obj - this may be either:
//
//             - a simple static function which is invoked with a pointer to each
//               element in the specified range.  The function must be of the form*:
//
...
//
//               This invocation operator will be invoked with a pointer to each element
//               in the specified range.  The 'function object' may contain additional
//               working info, call methods of other objects, etc.  This is difficult to
//               do with a simple static function - esp. if thread safety is important.
//
//               A side note: the 'function object' must have external scope - i.e. its
//               class may not be defined locally [nested] in the method calling this
//               method.  [However, the class can be defined just outside of the method
//               - hopefully using a namespace, etc. so that it is not global.]
//               CReis This may be MS Visual C++ specific and not ANSI C++ specific.
//
//             * [Actually, the return type is ignored and it may have more than one
//               argument provided that a default is provided for any additional
//               arguments.]
//
// Arg         pos - starting index position of elements to apply invoke_obj to
// Arg         elem_count - number of elements to apply invoke_obj to.  If elem_count is
//             ALength_remainder, the number of elements freed = length - pos.
//             (Default ALength_remainder)
// # Examples: array.apply(scan_elems);
// # See:      apply_method()
// # Notes:    This method performs index range checking when A_BOUNDS_CHECK is defined.
//             If an index is out of bounds, a AEx<ArrayBase<>> exception is thrown.
//             A_BOUNDS_CHECK is defined by default in debug mode and turned off in
//             release mode.
// # Author(s): Conan Reis
template<class _ElementType>
...
//---------------------------------------------------------------------------------------
// Calls the supplied non-const "method_m" method on "elem_count" elements
//             starting at index "pos".
// Arg         method_m - method address of a method available to _ElementType.  It must
//             take no arguments and return no arguments.
// Arg         pos - starting index position of elements to call method_m on.
// Arg         elem_count - number of elements to call method_m on.  If elem_count is
//             ALength_remainder, the number of elements freed = length - pos.
...
//---------------------------------------------------------------------------------------
// Calls the supplied constant "method_m" method on "elem_count" elements
//             starting at index "pos".
// Arg         method_m - method address of a method available to _ElementType.  It must
//             take no arguments and return no arguments.
// Arg         pos - starting index position of elements to call method_m on.
