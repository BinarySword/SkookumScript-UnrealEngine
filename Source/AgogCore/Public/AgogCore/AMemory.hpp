...
#define SS_NEW(_CLASS_NAME)   new (#_CLASS_NAME) _CLASS_NAME
...
#define SS_NEW_OPERATORS(_CLASS_NAME)   A_NEW_OPERATORS(_CLASS_NAME)
...
class AMemory
  ...
  // Class Methods
    ...
    static _ObjectType *  new_array(size_t num_objects, const char * name_p);
...
template<typename _ObjectType>
inline _ObjectType * AMemory::new_array(size_t num_objects, const char * name_p)
  {
  // Get memory
  ...
  for (size_t i = 0; i < num_objects; ++i)
    {
    new ((void*)(array_p + i)) _ObjectType;
    }
  return array_p;
  }
