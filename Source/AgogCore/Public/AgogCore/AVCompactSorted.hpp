...
inline AVCompactSorted<_ElementType, _KeyType, _CompareClass>::AVCompactSorted(
  ...
  tAVCompactArrayBase(sorted.m_count, AVCompactArrayBase::alloc_array(sorted.m_count))
  {
  if (sorted.m_count)
    {
...
inline AVCompactSorted<_ElementType, _KeyType, _CompareClass>::~AVCompactSorted()
  {
  AVCompactArrayBase::dtor_elems(this->m_array_p, this->m_count);
  AVCompactArrayBase::free_array(this->m_array_p);
  }
...
AVCompactSorted<_ElementType, _KeyType, _CompareClass>::operator=(
  ...
  if (this->m_count != new_count)
    ...
    if (new_count)
      ...
      _ElementType * array_p     = AVCompactArrayBase::alloc_array(new_count);
      _ElementType * elems_p     = sorted.m_array_p;
    ...
    for (; elems_pp < elems_end_pp; elems_pp++)
      ...
      if (!find(**elems_pp, AMatch_first_found, &pos))
        ...
        if (size < length + 1u)  // Needs more space
          ...
          this->m_size    = size;
          array_pp        = AVCompactArrayBase::alloc_array(this->m_size);
          ...
          AVCompactArrayBase::free_array(old_array_pp);
          }
