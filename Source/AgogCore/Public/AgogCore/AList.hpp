...
class AListFree : public AList<_ElementType, _NodeIdType>
  ...
  // All the AList<> constructors are hidden, so make appropriate links
    ...
    AListFree()                                              {}
    ~AListFree()                                             { AList::free_all(); }
    AListFree(AList<_ElementType, _NodeIdType> * list_p)     : AList<_ElementType, _NodeIdType>(list_p) {}

  protected:
  // Internal Methods

    AListFree(const AList<_ElementType, _NodeIdType> & list) : AList<_ElementType, _NodeIdType>(list) {}
    AListFree(const AListFree & list)                        : AList<_ElementType, _NodeIdType>(list) {}
...
//---------------------------------------------------------------------------------------
// Insert element at the tail/end of the list if the element is not already
//             in the list.  *** Uses a simple linear search, so can be slow on larger
//             lists.
...
//---------------------------------------------------------------------------------------
// Remove all elements and ensure their next and previous pointers are
//             properly reset.
// # Author(s): Conan Reis
template<class _ElementType, class _NodeIdType>
...
//             Looks in order from head to tail - so use if element is more likely to be
//             closer to the head than the tail.
