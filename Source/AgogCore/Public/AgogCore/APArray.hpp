...
template<
  ...
  // Modifying Behaviour methods
    ...
    void           append_all(const APArray & array, uint32_t start_pos, uint32_t end_pos = ALength_remainder);
...
// # Author(s):  Conan Reis
template<class _ElementType, class _KeyType, class _CompareClass>
inline _ElementType * & APArray<_ElementType, _KeyType, _CompareClass>::operator[](int pos)
  {
  APARRAY_BOUNDS_CHECK(uint32_t(pos));
