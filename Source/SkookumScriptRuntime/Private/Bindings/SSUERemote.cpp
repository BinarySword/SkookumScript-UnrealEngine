...
namespace
{
  const int32_t SSUERemote_ide_port = 12357;
...
// 
// #Author(s): Conan Reis
void SSUERemote::process_incoming()
  {
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
// 
// #Author(s): Conan Reis
AString SSUERemote::get_socket_str()
  {
  if (!is_connected())
    {
...
// 
// #Author(s): Conan Reis
TSharedPtr<FInternetAddr> SSUERemote::get_ip_address_local()
  {
  ISocketSubsystem * socket_system_p = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);
...
// 
// #Author(s): Conan Reis
bool SSUERemote::is_connected() const
  {
...
// 
// #Author(s): Conan Reis
void SSUERemote::set_mode(eSSLocale mode)
  {
  if (m_mode != mode)
    {
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if (m_socket_p)
      {
      SSDebug::print(a_str_format("SkookumScript: Disconnecting... %s\n", get_socket_str().as_cstr()), SSLocale_local);

      ISocketSubsystem * socket_system_p = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);

      if (!m_socket_p->Close())
        {
        SSDebug::print(a_str_format("  error closing socket: %i\n", (int32)socket_system_p->GetLastErrorCode()), SSLocale_local);
        }
    ...
    switch (mode)
      {
      case SSLocale_embedded:
        set_connect_state(ConnectState_disconnected);
        SSDebug::print("\nSkookumScript: Skookum IDE not connected (off-line)\n\n", SSLocale_local);
        break;

      case SSLocale_runtime:
        {
        SSDebug::print("SkookumScript: Attempting to connect to remote IDE\n", SSLocale_local);
        ...
        if (m_socket_p)
          ...
          local_addr->SetPort(SSUERemote_ide_port);
        ...
        if (!success)
          ...
          set_mode(SSLocale_embedded);
          return;
          }

        SSDebug::print(a_str_format("SkookumScript: Connected %s\n", get_socket_str().as_cstr()), SSLocale_local);
...
// #Modifiers: virtual
// #Author(s): Conan Reis
void SSUERemote::on_cmd_send(const ADatum & datum)
  {
  if (is_connected())
    {
    int32 bytes_sent = 0;
...
//---------------------------------------------------------------------------------------
double SSUERemote::get_elapsed_seconds()
  {
  return FPlatformTime::Seconds() - GStartTime;
  }

//---------------------------------------------------------------------------------------
void SSUERemote::wait_for_update()
  {
...
//---------------------------------------------------------------------------------------
bool SSUERemote::spawn_remote_ide()
  {
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // Look for Skookum IDE in game/project plug-in folder first.
  FString ide_path(
    FPaths::GamePluginsDir()
    / TEXT("SkookumScript/SkookumIDE/SkookumIDE") TEXT(SS_BITS_ID) TEXT(".exe"));
  ...
  if (!ide_exists)
    ...
    FString ide_engine_path(
      FPaths::EnginePluginsDir() / TEXT("SkookumScript/SkookumIDE/SkookumIDE") TEXT(SS_BITS_ID) TEXT(".exe"));
...
//---------------------------------------------------------------------------------------
AString SSUERemote::get_project_path()
  {
