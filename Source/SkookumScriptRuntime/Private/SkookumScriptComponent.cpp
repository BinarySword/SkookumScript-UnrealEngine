  ...
  if (!m_instance_p)
    ...
    if (!class_name.IsEmpty())
      ...
      if (class_p)
        ...
        {
        class_p = SSBrain::get_class(ASymbol_Actor); // Recover from bad user input
        }
      }
    else
      {
      SSClass ** class_pp = SSUE::g_class_map_u2s.Find(actor_p->GetClass());
      SS_ASSERTX(class_pp, a_cstr_format("Class '%S' of actor '%S' owning SkookumScriptComponent '%S' is unknown to SkookumScript!", *actor_p->GetClass()->GetName(), *actor_p->GetName(), *GetName()));
      class_p = *class_pp;
      }
