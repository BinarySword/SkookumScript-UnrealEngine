...
#include "Bindings/SSUERuntime.hpp"
#include "Bindings/SSUERemote.hpp"
...
class FSkookumScriptRuntime : public ISkookumScriptRuntime, public FTickableGameObject
  ...
  protected:
    ...
    SSUERuntime  m_runtime;

    #ifdef SKOOKUM_REMOTE_UNREAL
      SSUERemote m_remote_client;
    #endif
...
FSkookumScriptRuntime::FSkookumScriptRuntime()
  ...
  {
  //m_runtime.set_compiled_path("Scripts" SS_BITS_ID "\\");
  }
...
void FSkookumScriptRuntime::OnWorldInitPre(UWorld * world_p, const UWorld::InitializationValues init_vals)
  ...
  SSUERemote::ms_client_p->ensure_connected();
...
void FSkookumScriptRuntime::set_game_world(UWorld * world_p)
  ...
  if (obj_class_p->get_class_data().find(ASymbolX_c_world, AMatch_first_found, &var_pos))
    {
    obj_class_p->get_class_data().get_at(var_pos)->set_data(world_p ? SSUE::get_object_instance(world_p) : SSBrain::ms_nil_p);
    }
  else
    {
