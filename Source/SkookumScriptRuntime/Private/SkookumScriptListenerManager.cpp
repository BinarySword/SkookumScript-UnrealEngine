...
SkookumScriptListenerManager * SkookumScriptListenerManager::get_singleton()
  {
  SSUERuntime * runtime_p = SSUERuntime::get_singleton();
...
USkookumScriptListener * SkookumScriptListenerManager::alloc_listener(UObject * obj_p, SSInvokedCoroutine * coro_p, USkookumScriptListener::tUnregisterCallback callback_p)
  {
...
void SkookumScriptListenerManager::free_listener(USkookumScriptListener * listener_p)
  ...
  if (m_active_list.pop(*listener_p))
    ...
    {
    SS_ERRORX("SkookumScriptListener not found in active list.");
    }
  }
